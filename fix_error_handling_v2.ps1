...
$path = "C:\laragon\www\PPID\ppid-laravel\production_deployment\ppid_version2\jurnal_copy2.docx"
...
foreach ($para in $doc.Paragraphs) {
    ...
    if ($text -match "Namun, terdapat beberapa temuan kritis terkait aspek usability") {
    ...
    if ($text -match "However, there are some critical findings related to usability aspects") {
    ...
    if ($text -match "memberikan skor terendah \(skor 1\) pada kejelasan pesan kesalahan") {
    ...
    if ($text -match "Namun, dari aspek usability, sistem ini masih memiliki kelemahan kritis") {
    ...
    if ($text -match "Indikator produktivitas diukur menggunakan pilihan biner") {
...
if ($selection.Find.Execute()) {
    ...
    $nextPara = $para.Next()
    $para.Range.Delete()
    if ($nextPara -and $nextPara.Range.Text.Contains("pesan kesalahan yang kurang jelas")) {
