/**
 * JavaScript untuk form create dan edit pimpinan di frontend
 */

document.addEventListener('DOMContentLoaded', function() {
    // Initialize all form functionality
    initializeTabs();
    // initializeOrganizationField(); // This field is not directly editable in the frontend for pimpinan
...
function initializeSpouseField() {
    ...
    const spouseNameField = document.getElementById('spouse_name_field_family'); // Adjusted ID for frontend
    const spouseNameLabel = document.getElementById('spouse_name_label_family'); // Adjusted ID for frontend
    ...
    updateSpouseLabel();
    toggleSpouseVisibility(); // Call initially to set correct state
}
...
function initializeChildren() {
            ...
            removeButton.addEventListener('click', function(e) {
                ...
                if (childItem) {
                    ...
                    if (childItems.length > 1) {
                        ...
                        // Also re-index if it's the last one being cleared
                        updateChildrenIndices();
                    }
                }
            });
    ...
    // Initialize indices on page load if there are existing children
    if (childrenContainer.querySelectorAll('.child-item').length > 0) {
        updateChildrenIndices();
    }
}
...
function initializeCareerHistory() {
    ...
    if (careerContainer.querySelectorAll('.career-item').length > 0) {
        updateCareerIndices();
    }
}
...
function initializeEducation() {
    ...
    // Initialize indices on page load
    if (educationContainer.querySelectorAll('.education-item').length > 0) {
        updateEducationIndices();
    }
}
...
function initializeTraining() {
    ...
    // Initialize indices on page load
    if (trainingContainer.querySelectorAll('.training-item').length > 0) {
        updateTrainingIndices();
    }
}
...
function initializeOrganizationalHistory() {
    ...
    // Initialize indices on page load
    if (organizationalContainer.querySelectorAll('.organizational-item').length > 0) {
        updateOrganizationalIndices();
    }
}
...
function initializeAwards() {
    ...
    // Initialize indices on page load
    if (awardContainer.querySelectorAll('.award-item').length > 0) {
        updateAwardIndices();
    }
}
