                    ...
                    <div class="flex flex-col lg:flex-row gap-4 items-stretch lg:items-end bg-white/80 backdrop-blur-md p-6 rounded-[2.5rem] shadow-xl shadow-blue-500/5 border border-white mb-6">
                        ...
                        <div class="grid grid-cols-2 md:grid-cols-4 gap-3 lg:w-auto">
                            ...
                            <div class="min-w-[120px]">
                                ...
                                <select name="sort" onchange="updateContent()" class="w-full px-4 py-4 text-xs border-none bg-gray-50/50 rounded-2xl focus:ring-4 focus:ring-blue-500/10 font-bold text-gray-600 appearance-none">
                            ...
                            <div class="min-w-[100px]">
                                ...
                                <select name="per_page" onchange="updateContent()" class="w-full px-4 py-4 text-xs border-none bg-gray-50/50 rounded-2xl focus:ring-4 focus:ring-blue-500/10 font-bold text-gray-600 appearance-none">
                                    @foreach($perPageOptions as $opt)
                                        <option value="{{ $opt['value'] }}" {{ request('per_page') == $opt['value'] ? 'selected' : '' }}>{{ $opt['label'] }}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>

                        <!-- Search Button (Visual Only since its AJAX) -->
...
<script>
    ...
    async function updateContent(url = null) {
        ...
        // Remove duplicate keys (common with hidden inputs + checkboxes)
        const uniqueParams = new URLSearchParams();
        params.forEach((value, key) => {
            if (!uniqueParams.has(key)) {
                // If the key is filter_unit or sort_created, we want the LAST value (the checkbox one if it exists)
