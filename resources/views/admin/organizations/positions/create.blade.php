...
@section('title', 'Tambah Jabatan Organisasi')
...
@section('content')
    <div class="bg-white rounded-xl shadow p-6">
        ...
        <form action="{{ route('admin.organizations.positions.store', $organization) }}" method="POST">
            @csrf
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <input type="number" name="order_number" id="order_number" value="{{ old('order_number', 0) }}" min="0"
                           class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500">
                    @error('order_number')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
