...
@section('content')
    <div class="bg-white rounded-xl shadow p-6">
        <div class="mb-6">
            ...
            <p class="text-gray-600">Perbarui informasi slider</p>
        </div>
        ...
        <form action="{{ route('admin.sliders.update', $slider) }}" method="POST" enctype="multipart/form-data" id="editSliderForm">
            @csrf
            @method('PUT')
            <div class="mb-6">
                <label for="title" class="block text-sm font-medium text-gray-700 mb-2">Judul</label>
                <input type="text" name="title" id="title" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-blue-500 focus:border-blue-500 transition" value="{{ old('title', $slider->title) }}" required>
            </div>

            <div class="mb-6">
                <label class="block text-sm font-medium text-gray-700 mb-2">Tampilkan Judul</label>
                <div class="flex items-center">
                    <input type="checkbox" name="show_title" id="show_title" value="1" class="h-4 w-4 text-blue-600 border-gray-300 rounded focus:ring-blue-500" {{ old('show_title', $slider->show_title) ? 'checked' : '' }}>
            ...
            <div class="mb-6">
                ...
                <textarea name="description" id="description" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-blue-500 focus:border-blue-500 transition" rows="5">{{ old('description', $slider->description) }}</textarea>
            ...
            <div class="mb-6">
                ...
                <div class="flex items-center">
                    <input type="checkbox" name="show_description" id="show_description" value="1" class="h-4 w-4 text-blue-600 border-gray-300 rounded focus:ring-blue-500" {{ old('show_description', $slider->show_description) ? 'checked' : '' }}>
            ...
            <div class="mb-6">
                ...
                <select name="category" id="category" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-blue-500 focus:border-blue-500 transition">
                    ...
                    @foreach($categories as $category)
                        <option value="{{ $category }}" {{ old('category', $slider->category) == $category ? 'selected' : '' }}>{{ $category }}</option>
                    @endforeach
                </select>
            </div>
            ...
            <div class="mb-6" id="jenisDokumenWrapper">
                ...
                <select name="jenis_dokumen" id="jenis_dokumen" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-blue-500 focus:border-blue-500 transition"
                    ...
                    @foreach($jenis_dokumen as $value => $label)
                        ...
                        @endphp
                        <option value="{{ $value }}" data-desc="{{ $desc }}" {{ old('jenis_dokumen', $slider->jenis_dokumen) == $value ? 'selected' : '' }}>{{ $value }}</option>
                    @endforeach
                </select>
            ...
            <div class="mb-6">
                ...
                <input type="file" name="image" id="image" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-blue-500 focus:focus:border-blue-500 transition">
                <p class="mt-2 text-sm text-gray-500">Biarkan kosong untuk tetap menggunakan gambar saat ini</p>
            </div>

            <div class="mb-6">
                <label class="block text-sm font-medium text-gray-700 mb-2">Gambar Saat Ini</label>
                <img src="{{ asset('storage/' . $slider->image) }}" alt="{{ $slider->title }}" class="h-32 w-auto object-cover rounded border border-gray-200">
            </div>

            <div class="mb-6">
                <label class="block text-sm font-medium text-gray-700 mb-2">Status</label>
                <div class="flex items-center">
                    <input type="checkbox" name="active" id="active" value="1" class="h-4 w-4 text-blue-600 border-gray-300 rounded focus:ring-blue-500" {{ old('active', $slider->active) ? 'checked' : '' }}>
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function () {
            ...
            const jenisDokumenWrapper = document.getElementById('jenis_dokumen_desc').parentElement;
