        ...
        <form action="{{ route('admin.ai-settings.update', $aiSetting) }}" method="POST">
            ...
            <div class="space-y-4">
                ...
                <div>
                    ...
                    <input type="text" name="model" id="model" value="{{ old('model', $aiSetting->model) }}" required class="w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500">
                    @error('model')
                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    @enderror
                </div>
