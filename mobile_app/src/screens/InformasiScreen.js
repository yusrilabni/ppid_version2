...
import { FileText, Calendar, Building, ChevronRight, ChevronLeft, Archive, CheckCircle2, Search, X, Clock } from 'lucide-react-native';
...
export default function InformasiScreen() {
  ...
  }, [searchQuery, selectedStatus, fetchDocuments]);
        ...
        <View style={[styles.cardContainer, isArsip && styles.cardArsipBg]}>
          <View style={styles.cardTop}>
            ...
            <View style={styles.headerInfo}>
                <Text style={[styles.categoryTxt, isArsip && styles.txtArsipSub]}>{item.category || 'DOKUMEN'}</Text>
                <View style={[styles.badge, isArsip ? styles.badgeArsip : styles.badgeBerlaku]}>
                    <Text style={[styles.badgeTxt, isArsip ? styles.badgeTxtArsip : styles.badgeTxtBerlaku]}>{item.status}</Text>
                </View>
            </View>
          </View>
          <Text style={[styles.titleTxt, isArsip && styles.txtArsipMain]} numberOfLines={2}>{item.title}</Text>
          <View style={styles.divider} />
          <View style={styles.cardBottom}>
            <View style={styles.metaGroup}>
                <View style={styles.metaItem}>
                    <Clock size={12} color="#94a3b8" />
                    <Text style={styles.metaTxt}>{item.tanggal_upload || 'Baru'}</Text>
                </View>
                <View style={[styles.metaItem, { flex: 1 }]}>
                    <Building size={12} color="#94a3b8" />
                    <Text style={styles.metaTxt} numberOfLines={1}>{item.organization_name}</Text>
                </View>
            </View>
            <View style={styles.actionBtn}><ChevronRight size={16} color="#2563eb" /></View>
          </View>
        </View>
      </TouchableOpacity>
    );
    ...
    <View style={styles.container}>
      ...
      <View style={[styles.headerSection, { paddingTop: STATUSBAR_HEIGHT + 10 }]}>
          <View style={styles.headerTop}>
            ...
            <View style={styles.countBadge}><Text style={styles.countTxt}>{totalData}</Text></View>
          </View>
          <View style={styles.searchRow}>
            <View style={styles.searchContainer}>
                <Search size={18} color="#94a3b8" />
                <TextInput style={styles.searchInput} placeholder="Cari dokumen..." placeholderTextColor="#94a3b8" value={searchQuery} onChangeText={setSearchQuery} />
          ...
          <ScrollView horizontal showsHorizontalScrollIndicator={false} style={styles.filterScroll}>
            {['Semua', 'BERLAKU', 'ARSIP'].map((status) => (
                <TouchableOpacity key={status} onPress={() => setSelectedStatus(status)} style={[styles.filterTab, selectedStatus === status && styles.filterTabActive]}>
                    <Text style={[styles.filterLabel, selectedStatus === status && styles.filterLabelActive]}>{status}</Text>
                </TouchableOpacity>
            ))}
          </ScrollView>
      </View>
        ...
        <FlatList
          ...
          ListFooterComponent={lastPage > 1 && (
            <View style={styles.pagination}>
                <TouchableOpacity disabled={currentPage === 1} onPress={() => fetchDocuments(currentPage - 1, searchQuery, selectedStatus)} style={[styles.pageBtn, currentPage === 1 && styles.btnDisabled]}><ChevronLeft size={20} color="#fff" /></TouchableOpacity>
                <View style={styles.pageInfo}><Text style={styles.pageInfoTxt}>{currentPage} / {lastPage}</Text></View>
                <TouchableOpacity disabled={currentPage === lastPage} onPress={() => fetchDocuments(currentPage + 1, searchQuery, selectedStatus)} style={[styles.pageBtn, currentPage === lastPage && styles.btnDisabled]}><ChevronRight size={20} color="#fff" /></TouchableOpacity>
            </View>
          )}
        />
      )}
...
const styles = StyleSheet.create({
  ...
  cardContainer: { flex: 1, backgroundColor: '#fff', borderRadius: 25, padding: 20, marginLeft: 3, elevation: 5 },
  cardArsipBg: { backgroundColor: '#f8fafc' },
  ...
  categoryTxt: { fontSize: 10, fontWeight: '900', color: '#2563eb' },
  ...
  pageBtn: { backgroundColor: '#1e293b', padding: 12, borderRadius: 15 },
